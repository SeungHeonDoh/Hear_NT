...
class MelFilterbanks(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self._config_window_len = window_len
    self._config_window_stride = window_stride
  ...
  def get_config(self):
    config = super(MelFilterbanks, self).get_config()
    config.update({"n_filters": self._n_filters})
    # config.update({"window_len": 46})
    # config.update({"window_stride": 23})
    config.update({"window_len": self._config_window_len})
    config.update({"window_stride": self._config_window_stride})
    config.update({"sample_rate": self._sample_rate})

    # param_dict = {}
    # param_dict['n_filters'] = self._n_filters
    # param_dict['window_len'] = self._window_len
    # param_dict['window_stride'] = self._window_stride
    # param_dict['sample_rate'] = self._sample_rate
